...
#[no_mangle]
pub extern fn html5ever_parse_document(html: *const c_char, html_size: size_t, use_arena: c_int) -> c_int {
    if html.is_null() {
        return C_RES_ERR_INVALID_PARAMETER;
    }

    let html_slice = unsafe { std::slice::from_raw_parts(html as *const u8, html_size as usize) };
    ...
    if use_arena == 0 {
        ...
        html5ever_parse_slice_into_arena(html_slice, &arena);
    }

    C_RES_OK
}

// copied from https://github.com/servo/html5ever/pull/288
...
fn html5ever_parse_slice_into_arena<'a>(bytes: &[u8], arena: Arena<'a>) -> Ref<'a> {
    ...
    html5ever::parse_document(sink, Default::default()).from_utf8().one(bytes)
}
